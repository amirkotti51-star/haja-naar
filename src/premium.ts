...
type Service = { id:string; name:string; city?:string; premium?:boolean; featured?:boolean; expiresAt?:string; active?:boolean };
...
async function openPremium(){
  ...
  const root=document.createElement('div');root.className='px-backdrop';root.innerHTML=`<section class="px-modal" role="dialog" aria-modal="true" aria-label="Premium">
    ...
    <div class="px-plans">${plans.map((p,i)=>`<button type="button" class="px-plan ${i===1?'selected':''}" data-plan="${p.key}">${p.badge?`<span class="px-plan-badge">${p.badge}</span>`:''}<h3>${p.name}</h3><div class="px-price">${p.price}<small> USDT</small></div><p>${p.note}</p><small>${p.days} يوم من Premium</small></button>`).join('')}</div>
    ...
    <button id="px-submit" class="px-submit">الدفع الآمن عبر Binance Pay</button>
    <div class="px-binance"><span>◈</span><span><strong>Binance Pay</strong><br>الدفع يتم بالـ USDT عبر صفحة Binance الآمنة.</span></div>
    <div class="px-benefits"><div class="px-benefit"><b>★ ظهور مميز</b><span>تظهر خدمتك قبل الخدمات العادية عند ترتيب النتائج.</span></div><div class="px-benefit"><b>✓ شارة Premium</b><span>هوية بصرية واضحة تساعد العميل على اكتشافك بسرعة.</span></div><div class="px-benefit"><b>⚡ تفعيل سريع</b><span>بعد تأكيد الدفع يتم تفعيل المدة تلقائيًا.</span></div></div>
    <div class="px-foot">لا نحفظ بيانات محفظتك أو بيانات الدفع داخل خِدمة TN. مفاتيح Binance تبقى على الخادم فقط.</div>
  </section>`;
  ...
  try{
    const r=await api.get('/api/my-services');const mine=(r.data.services||[]) as Service[];const premium=mine.filter(activePremium);
    ...
    select.innerHTML=available.length?available.map(s=>`<option value="${esc(s.id)}">${esc(s.name)}${s.city?` — ${esc(s.city)}`:''}</option>`).join(''):'<option value="">لا توجد خدمة مؤهلة حاليًا</option>';
    const submit=root.querySelector('#px-submit') as HTMLButtonElement;submit.disabled=!available.length;
    submit.addEventListener('click',async()=>{if(!select.value)return;submit.disabled=true;submit.textContent='جاري فتح Binance Pay…';try{const response=await api.post('/api/binance/create',{serviceId:select.value,plan:selected});const payUrl=response.data?.payUrl||response.data?.universalUrl;if(payUrl){location.href=payUrl;return;}throw new Error(response.data?.error||'تعذر إنشاء الدفع');}catch(e){alert(e instanceof Error?e.message:'تعذر بدء الدفع. حاول مرة أخرى.');submit.disabled=false;submit.textContent='الدفع الآمن عبر Binance Pay';}});
  }catch{current.innerHTML='<div class="px-current"><strong>تعذر تحميل خدماتك.</strong><small>حاول مرة أخرى.</small></div>';}
}

function addPremiumButton(){if(document.getElementById('premium-launcher'))return;const b=document.createElement('button');b.id='premium-launcher';b.setAttribute('aria-label','فتح Premium');b.textContent='♛ Premium';b.style.cssText='position:fixed;right:20px;bottom:20px;z-index:11000;border:1px solid #fbbf24aa;border-radius:999px;padding:12px 18px;background:linear-gradient(135deg,#111827,#3b2a0a);color:#fcd34d;font-weight:950;box-shadow:0 14px 35px #0006;cursor:pointer;direction:rtl';b.addEventListener('click',openPremium);document.body.appendChild(b);}
